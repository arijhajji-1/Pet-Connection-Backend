...
var captureShot = () => {
  ...
  return new Promise(resolve => {
    ...
    var img = `./public/new_uploads/capture_user.${options.output}`
...
const regenize2 = async (req, res) => {
  ...
  captureShot().then(async (imagePath) => {
    ...
    axios.post('http://127.0.0.1:5000/face_detection', formData, {
      headers: {
      ...
      .then(response => {
        ...
        // res.sendStatus(200).send(response.result);
        res.status(200).send(response.data)
      })
      .catch(error => {
        console.log("error", error);
        // res.sendStatus(500); // ou tout autre code d'erreur que vous souhaitez renvoyer
        res.status(404).send("erreur en flask")
    ...
    // res.sendStatus(500); // ou tout autre code d'erreur que vous souhaitez renvoyer
    res.status(404).send("erreur en express js")
...
const loginFaceRecognition = async (req, res) => {
  ...
  // const user = await User.findById("641a3529b5b44c63f77b3555")
...
module.exports = {
  register, login, profile, getAll, deleteUser, banUser, logout,
  twofactorverification, enableTwoFactor, disableTwoFactor, facebooklogin, loginGoogle,
  promoteUser, upload, getUserImage, updateuser, updateUser, deleteUser, banUser, addUser,
  banUser2, updateUserPasswordCtrl, forgetPasswordToken, passwordResetCtrl, verifyUser,
  updateuseradmin, getuserInfo, uploadd, regenize2,loginFaceRecognition
}
